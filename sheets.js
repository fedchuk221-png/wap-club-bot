...
async function registerReferral(chatId, name, referrerChatId) {
  try {
    await ensureSheetExists(CLIENTS_SHEET_NAME, CLIENTS_HEADERS);
    if (!clientsCache) await loadClientsCache();
    if (clientsCache.map.has(String(chatId))) return false; // вже існує — не перереєструємо
    const sheets = await getSheets();
    const newCode = (clientsCache.maxCode || 0) + 1;
    await withRetry(() =>
      sheets.spreadsheets.values.append({
...
// Для LTV-будильника: усі клієнти з ChatId (для розсилки), без зайвих полів.
async function getAllClientsWithChatId() {
  if (!clientsCache) await loadClientsCache();
  return Array.from(clientsCache.map.entries()).map(([chatId, rec]) => ({ chatId, ...rec }));
}
