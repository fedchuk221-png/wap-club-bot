...
module.exports = { isEnabled, uploadTelegramPhoto, backupSpreadsheet };

// Щотижнева резервна копія самої таблиці (Drive: files.copy) — не залежить від
// того, чи включено фото-модуль (GOOGLE_DRIVE_FOLDER_ID), працює завжди,
// поки в сервісного акаунта є доступ до Sheets API.
async function backupSpreadsheet(spreadsheetId, nameSuffix) {
  try {
    const drive = await getDrive();
    const meta = await drive.files.get({ fileId: spreadsheetId, fields: "name" });
    const copyName = `${meta.data.name} — backup ${nameSuffix}`;
    const requestBody = { name: copyName };
    if (FOLDER_ID) requestBody.parents = [FOLDER_ID];
    const copy = await drive.files.copy({ fileId: spreadsheetId, requestBody });
    return copy.data.id || null;
  } catch (e) {
    console.error("backupSpreadsheet failed:", e.message || e);
    return null;
  }
}
