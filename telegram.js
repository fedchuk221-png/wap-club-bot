...
async function tgApi(method, payload) {
  try {
    ...
    if (!res.ok) {
      ...
      // "message is not modified" — не помилка по суті: Telegram відмовляється
      // редагувати повідомлення на ідентичний вміст (наприклад, повторний клік
      // тієї самої кнопки без реальних змін). Не засмічуємо лог справжніми помилками.
      if (/message is not modified/i.test(bodyText)) {
        return res;
      }
      console.error(`TG API error [${method}]:`, bodyText);
      // Защита от "тихой смерти" сообщения: если Telegram отклонил его именно
      // из-за разбора Markdown (спецсимвол в номере/названии/дате и т.п.) —
      // повторяем тот же запрос без parse_mode, простым текстом, лишь бы дошло.
      if (payload.parse_mode && /can't parse entities/i.test(bodyText) && (method === "sendMessage" || method === "editMessageText")) {
        console.error(`Retrying [${method}] without parse_mode after Markdown parse error`);
        const plainPayload = { ...payload };
        delete plainPayload.parse_mode;
        try {
          const retryRes = await fetch(`${BASE}/${method}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(plainPayload)
          });
          if (!retryRes.ok) {
            console.error(`Retry [${method}] also failed:`, await retryRes.text());
          }
          return retryRes;
        } catch (e2) {
          console.error(`Retry [${method}] exception:`, e2);
        }
      }
    }
    return res;
...
function sendDice(chatId, emoji) {
  return tgApi("sendDice", { chat_id: String(chatId), emoji });
}
