...
// 🎰 Колесо фортуни — приз завжди виражений у бонусних гривнях (та ж система, що й реферальні бонуси)
const WHEEL_PRIZES = [
  { label: "🎁 Безкоштовне чорніння шин на наступний виїзд", weight: 30, bonus: 100 },
  { label: "💰 Знижка на автопарфюм з вітрини", weight: 30, bonus: 50 },
  { label: "💎 Джекпот! Бонусний рахунок +200 грн", weight: 10, bonus: 200 },
  { label: "🍀 Не пощастило — спробуйте наступного разу!", weight: 30, bonus: 0 }
];
...
async function verifyOrderRowIntegrity(orderId, index) {
  ...
  if (!row || row[0] !== orderId) {
    const msg = `🚨 УВАГА: лист "Заказы" пошкоджено! Заявка ${orderId} записалась не в ту колонку (можливо, хтось вручну вставив/видалив стовпець). Перевірте структуру таблиці — стовпці A-N мають бути строго: ID, Дата, Клієнт, Телефон, Послуга, Сума, Статус, Оцінка, Фото До, Фото Після, Дата виконання, LTV, Джерело, Приз.`;
    console.error(msg);
...
async function handleCallback(cb) {
  ...
  if (data.startsWith("rate_")) {
    ...
    if (rating === 5) {
      ...
      responseKb = { inline_keyboard: [[{ text: "🎰 Крутити колесо фортуни!", callback_data: `wheel_${ordIdx}` }]] };
    } else if (rating <= 3) {
      responseText = d.review_thanks_low;
    } else {
      responseText = d.review_thanks.replace("{rating}", "⭐".repeat(rating));
    }
    await tg.ackAndEdit(cb.id, chatId, messageId, responseText, responseKb);
  ...
  if (data.startsWith("wheel_")) {
    ...
    if (!ord) { await tg.answerCallback(cb.id); return; }
    if (ord[13]) {
      await tg.ackAndEdit(cb.id, chatId, messageId, `🎰 Ви вже отримали приз по цьому замовленню: ${ord[13]}`);
      return;
    }
    await tg.answerCallback(cb.id);
    await tg.editMessage(chatId, messageId, "🎰 Крутимо колесо фортуни...");
    // Нативна анімація гральних кубиків Telegram — справжній спін, а не імітація правками тексту.
    await tg.sendDice(chatId, "🎰");

    const prize = pickWeightedPrize();
    await sheetsApi.updateOrderCell(ordIdx, 14, prize.label);
    sheetsApi.invalidateOrdersCache();
    if (prize.bonus > 0) {
      await sheetsApi.addBonusBalance(chatId, prize.bonus);
    }
    const resultText = prize.bonus > 0
      ? `🎉 *Вітаємо!*\n\n${prize.label}\n\n💰 На ваш бонусний рахунок нараховано +${prize.bonus} грн (можна списати до 50% наступного замовлення).`
      : `${prize.label}\n\nАле дякуємо, що ви з нами! 💙`;

    // Даємо анімації кубика дограти (у Telegram-клієнтах вона триває ~2 секунди) перед результатом.
    setTimeout(() => {
      tg.sendMessage(chatId, resultText).catch(() => {});
      if (ADMIN_ID) {
        tg.sendMessage(ADMIN_ID, `🎰 Клієнт по заявці №${ordIdx + 1} виграв: ${prize.label}`).catch(() => {});
      }
    }, 2500);
    return;
  }
...
async function renderClubPass(chatId, messageId, clientName) {
  ...
  if (sub && sub.status === "Активна") {
    ...
    kb = { inline_keyboard: [[{ text: "❌ Скасувати підписку", callback_data: "club_cancel" }], [{ text: "🔙 У головне меню", callback_data: "menu_main" }]] };
...
async function renderCompletedOrder(chatId, index, messageId, cbId) {
  ...
  const prizeLine = ord[13] ? `\n🎰 *Приз:* ${ord[13]}` : "";
  const text =
    `📁 *ЗАВЕРШЕНА ЗАЯВКА №${realIdx + 1}* (${position + 1} з ${completed.length})\n\n` +
    `📦 *ID:* \`${ord[0]}\`\n📅 *Дата:* ${ord[1]}\n👤 *Клієнт:* ${ord[2]}\n📞 *Телефон:* \`${ord[3]}\`\n` +
    `🛠 *Послуга:* *${ord[4]}*\n💰 *Сума:* *${ord[5]} грн*${ratingLine}${prizeLine}`;
