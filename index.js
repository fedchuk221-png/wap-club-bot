...
async function handleCallback(cb) {
  ...
  if (data.startsWith("pickclass_")) {
    ...
    if (calendarApi.isEnabled()) {
      kbRows.push([{ text: "📅 Обрати час запису", callback_data: `slots_${code}_${classCode}` }]);
    } else {
  ...
  // --- 📅 Календар: вибір слоту (тільки якщо GOOGLE_CALENDAR_ID заданий) ---
  if (data.startsWith("slots_")) {
    const parts = data.split("_");
    const code = parts[1];
    const classCode = parts[2];
    const slots = await calendarApi.getAvailableSlots(8);
    const prevState = store.getState(chatId) || {};
    store.setState(chatId, { ...prevState, offeredSlots: slots });
    if (!slots.length) {
      await tg.ackAndEdit(cb.id, chatId, messageId, "😔 На жаль, найближчим часом вільних слотів немає. Напишіть нам напряму: @zabrodni_y", {
        inline_keyboard: [[{ text: d.btn_back, callback_data: "menu_main" }]]
      });
      return;
    }
    const rows = slots.map((s, i) => [{ text: s.label, callback_data: `slotpick_${code}_${classCode}_${i}` }]);
    rows.push([{ text: d.btn_back, callback_data: `pickclass_${code}_${classCode}` }]);
    await tg.ackAndEdit(cb.id, chatId, messageId, "📅 *Оберіть зручний час:*", { inline_keyboard: rows });
    return;
  }
  if (data.startsWith("slotpick_")) {
    const parts = data.split("_");
    const code = parts[1];
    const classCode = parts[2];
    const i = parseInt(parts[3], 10);
    const prevState = store.getState(chatId) || {};
    const slot = (prevState.offeredSlots || [])[i];
    if (!slot) { await tg.answerCallback(cb.id); return; }
    store.setState(chatId, { ...prevState, slotStart: slot.startISO, slotEnd: slot.endISO, slotLabel: slot.label });
    const savedPhone = await sheetsApi.getClientPhone(chatId);
    const kbRows = [];
    if (savedPhone) {
      kbRows.push([{ text: d.btn_confirm_saved_phone.replace("{phone}", savedPhone), callback_data: `usesaved_${code}_${classCode}` }]);
    ...
    await tg.ackAndEdit(cb.id, chatId, messageId, `📅 Ви обрали: *${slot.label}*\n\nПідтвердіть замовлення:`, { inline_keyboard: kbRows });
    return;
  }
  ...
  if (String(chatId) === String(ADMIN_ID)) {
    ...
    if (data.startsWith("manualclass_")) {
      ...
      if (calendarApi.isEnabled()) {
        await renderManualSlotPicker(chatId, messageId, cb.id);
      } else {
        await renderManualConfirm(chatId, messageId, cb.id);
      }
      return;
    }
    if (data.startsWith("manualslot_")) {
      const i = parseInt(data.replace("manualslot_", ""), 10);
      const state = store.getState(chatId) || {};
      const slot = (state.offeredSlots || [])[i];
      const newData = { ...(state.data || {}), slot: slot || null };
...
async function renderManualSlotPicker(chatId, messageId, cbId) {
  const slots = await calendarApi.getAvailableSlots(8);
  const state = store.getState(chatId) || {};
  store.setState(chatId, { ...state, offeredSlots: slots });
  if (!slots.length) {
    await renderManualConfirm(chatId, messageId, cbId);
    return;
  }
  const rows = slots.map((s, i) => [{ text: s.label, callback_data: `manualslot_${i}` }]);
  rows.push([{ text: "⏭ Без запису на конкретний час", callback_data: "manualslotskip" }]);
  const text = "📅 *Оберіть час запису:*";
  if (cbId && messageId) await tg.ackAndEdit(cbId, chatId, messageId, text, { inline_keyboard: rows });
  else await tg.sendMessage(chatId, text, { inline_keyboard: rows });
}
